...
1. **Maple-tools primary (default)** (`EVPRTR_MAPLE_TOOLS_PRIMARY=1`): When the request has `tools` and `tool_choice` is not `none`, the compositor rewrites the Maple-bound prompt into HF/DeepGrove-style ``<tools>`` / ``<tool_call>`` markup (`EVPRTR_MAPLE_TOOL_MARKUP=1`, default on) and strips OpenAI ``tools`` from the upstream call. Valid native ``tool_calls`` still pass through verify (degenerate-arg correction via Needle when enabled). Pseudo ``<tool_call>`` markup is promoted deterministically; unparseable markup may ask Needle to structure. Non-empty prose without tools is presented as the final answer (no Needle). Empty/thin Maple falls back to **Needle structure** (`needle_structure_fallback`). Keyword “mutation task” classification, synthetic write forging, and `missing_mutation` verify are **retired**. Whole-completion ``structured_outputs`` grammar (`EVPRTR_TOOLS_GRAMMAR`, default **off**) remains insurance only. Before present, `read` on a clear directory path (`.`, trailing slash, or absolute `is_dir`) is coerced to `ls`.

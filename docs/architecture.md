...
1. **Maple-tools primary (default)** (`EVPRTR_MAPLE_TOOLS_PRIMARY=1`): When the request has `tools` and `tool_choice` is not `none`, the compositor rewrites the Maple-bound prompt into HF/DeepGrove-style ``<tools>`` / ``<tool_call>`` markup (`EVPRTR_MAPLE_TOOL_MARKUP=1`, default on) and strips OpenAI ``tools`` from the upstream call. Valid native ``tool_calls`` still pass through verify. Pseudo ``<tool_call>`` markup is promoted deterministically. Plain prose (no markup) returns as ``stop``. Empty/thin Maple or unparseable markup falls back to **Needle structure** (`needle_structure_fallback`). Whole-completion ``structured_outputs`` grammar (`EVPRTR_TOOLS_GRAMMAR`, default **off**) is insurance only. Before present, `read` on a clear directory path (`.`, trailing slash, or absolute `is_dir`) is coerced to `ls`.
